...
if __name__ == "__main__":
    ...
    for i in range(len(speeds)):
        for j in range(len(typeNumDom)):
            if j==2:
                file = "DATA/{0}/entropy{1}_MS_{2}.txt".format(typeNumDom[j], typeAVG,speeds[i])
            else:
    ...
    if legendON==1:
        plt.legend(("Q3D based-blade","F3D based-blade","F3D MS based-blade","Q3D re-design blade","F3D re-design blade","F3D MS re-design blade"))
