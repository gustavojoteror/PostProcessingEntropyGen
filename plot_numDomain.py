...
if __name__ == "__main__":
    ...
    for i in range(len(typeNumDom)):

        if(cont<2):
            file = "DATA/{0}/entropy{1}_{2}.txt".format(typeNumDom[i], typeAVG,speeds)
        else:
            file = "DATA/{0}/entropy{1}_MS_{2}.txt".format(typeNumDom[i], typeAVG,speeds)
        property=[]
        radius=[]
        cont +=1
